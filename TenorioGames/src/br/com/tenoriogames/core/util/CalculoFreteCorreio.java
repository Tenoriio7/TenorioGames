...
public class CalculoFreteCorreio {
	public static Double CalcularFrete(Venda venda){
		...
		String sCepOrigem = "08567450";
		String sCepDestino = venda.getUsuario().getEndereco().getCEP();
		String nVlPeso = venda.getPeso().toString();
		String nCdVenda = "1";
		String nVlComprimento = "20";
		String nVlDimen = "2";
		String nVlLargura = "15";
		String nVlDiametro = "3";
		String sCdMaoPropria = "s";
		String nVlValorDeclarado = "2";
		...
		parameters.setProperty("nCdFormato", nCdVenda);
		parameters.setProperty("nVlComprimento", nVlComprimento);
		parameters.setProperty("nVlAltura", nVlDimen);
		...
		try {
		    ...
		    for (int j = 0; j < nodes.getLength(); j++) {
		      ...
		      String aux = getCharacterDataFromElement(line);
		      System.out.println(aux);
		      aux=aux.replace(",", ".");
		      //int valorInt =  Integer.parseInt(aux);
		      ...
		      valorFinal  =  Double.parseDouble(aux);
		      aux=valorFinal.toString();
		      BigDecimal format = new BigDecimal(aux);
		      format =  format.round(new MathContext(4));
		      aux=format.toString();
		      valorFinal=Double.parseDouble(aux);
